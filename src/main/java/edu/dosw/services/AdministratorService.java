...
/**
 * Service class that provides business logic for managing Administrator entities.
 * This service handles operations such as creating, retrieving, updating, and deleting administrators.
 * It also includes functionality for generating administrator email addresses.
 *
 */
@AllArgsConstructor
@Service
public class AdministratorService {
    ...
    public Administrator getAdministratorById(String administratorId) {
        Administrator administrator = administratorRepository.findByUserId(administratorId).orElse(null);
    ...
    /**
     * Creates a new administrator with the provided information.
     * Validates that all required personal data is provided.
     *
    ...
    public Administrator createAdministrator(AdministratorDto administratorCreationRequest) {
        if (administratorCreationRequest.identityDocument() == null || administratorCreationRequest.fullName() == null){
        ...
        try{
            ...
            throw new BusinessException("An inesperated error has occurred when creating the administrator");
        }

    }

    /**
     * Generates an email address for an administrator based on their full name.
     * The email format is: firstname.lastnameInitial@admin.escuelaing.edu.co
     *
     ...
     */
    public Administrator updateAdministrator(String administratorId, AdministratorDto administratorUpdateRequest) {
        Administrator administrator = administratorRepository.findByUserId(administratorId).orElse(null);
        if (administrator == null){
            logger.error("Administrator not found");
            throw new BusinessException("Administrator not found");
        }
        if (administratorUpdateRequest.fullName() != null) administrator.setFullName(administratorUpdateRequest.fullName());
        if (administratorUpdateRequest.identityDocument() != null) administrator.setIdentityDocument(administratorUpdateRequest.identityDocument());

        try{
            return administratorRepository.save(administrator);
        }catch (Exception e){
            throw new BusinessException("An inesperated error has occurred when updating the administrator");
        }
    }
    ...
    public Administrator deleteAdministrator(String administratorId) {
        ...
        try{
            ...
            throw new BusinessException("An inesperated error has occurred when deleting the administrator");
        }
    }
}
