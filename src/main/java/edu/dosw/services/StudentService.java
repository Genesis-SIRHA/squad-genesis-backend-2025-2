...
/**
 * Service class that provides business logic for managing Student entities.
 * This service handles operations such as creating, retrieving, updating, and deleting students.
 * It also includes functionality for generating student email addresses.
 */
@AllArgsConstructor
@Service
public class StudentService {
     ...
     * Validates that all required personal and academic data is provided.
     *
    ...
    public Student createStudent(StudentDto studentCreationRequest) {
        if (studentCreationRequest.identityDocument() == null || studentCreationRequest.fullName() == null){
        ...
        try{
            ...
            throw new BusinessException("An inesperated error has occurred when creating the student: "+ e.getMessage());
        }

    }

    /**
     * Generates an email address for a student based on their full name.
     * The email format is: firstname.lastnameInitial@mail.escuelaing.edu.co
     *
    ...
    public Student updateStudent(String studentId, StudentDto studentUpdateRequest) {
        ...
        if (studentUpdateRequest.fullName() != null) student.setFullName(studentUpdateRequest.fullName());
        if (studentUpdateRequest.identityDocument() != null) student.setIdentityDocument(studentUpdateRequest.identityDocument());
        if (studentUpdateRequest.facultyName() != null) student.setFacultyName(studentUpdateRequest.facultyName());
        if (studentUpdateRequest.plan() != null) student.setPlan(studentUpdateRequest.plan());
        if (studentUpdateRequest.academicGrade() != null) student.setAcademicGrade(studentUpdateRequest.academicGrade());

        try{
            return studentRepository.save(student);
        }catch (Exception e){
            throw new BusinessException("An inesperated error has occurred when updating the student: "+ e.getMessage());
        }
    }
    ...
    public Student deleteStudent(String studentId) {
        ...
        try{
            ...
            throw new BusinessException("An inesperated error has occurred when deleting the student: "+ e.getMessage());
        }
    }
}
