...
/**
 * Service class that provides business logic for managing Dean entities.
 * This service handles operations such as creating, retrieving, updating, and deleting deans,
 * as well as retrieving faculty information associated with deans.
 */
@Service
@AllArgsConstructor
public class DeanService {
     ...
     * @throws BusinessException if required personal data is missing or an error occurs during creation
     */
    public Dean createDean(DeanDto deanCreationRequest) {
        ...
        Dean dean = new Dean.DeanBuilder()
        ...
        try{
            ...
            throw new BusinessException("An inesperated error has occurred when creating the dean: "+ e.getMessage());
        }

    }

    /**
     * Generates an email address for a dean based on their full name.
     * The email format is: firstname.lastnameInitial@escuelaing.edu.co
     *
    ...
    public Dean updateDean(String deanId, DeanDto deanUpdateRequest) {
        ...
        if (deanUpdateRequest.identityDocument() != null) dean.setIdentityDocument(deanUpdateRequest.identityDocument());
        if (deanUpdateRequest.facultyName() != null) dean.setFacultyName(deanUpdateRequest.facultyName());

        try{
            return deanRepository.save(dean);
        }catch (Exception e){
            logger.error("Error updating dean: {}", e.getMessage(), e);
            throw new BusinessException("An inesperated error has occurred when updating the dean: "+ e.getMessage());
        }
    }
    ...
    public Dean deleteDean(String deanId) {
        ...
        try{
            ...
            throw new BusinessException("An inesperated error has occurred when deleting the dean: "+ e.getMessage());
        }
    }
