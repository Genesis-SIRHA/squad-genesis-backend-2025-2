...
class RequestPeriodServiceTest {
  ...
  @Mock private RequestPeriodRepository requestPeriodRepository;

  @Mock private PeriodService periodService;

  @Mock private RequestPeriodValidator requestPeriodValidator;

  @InjectMocks private RequestPeriodService requestPeriodService;

  @Test
  void getActivePeriod_WithActivePeriod_ShouldReturnPeriod() {
    RequestPeriodDTO expectedPeriod =
  ...
  void getActivePeriod_WithoutActivePeriod_ShouldThrowResourceNotFoundException() {
    ...
    ResourceNotFoundException exception =
        assertThrows(ResourceNotFoundException.class, () -> requestPeriodService.getActivePeriod());

    assertEquals(
        "There is no active period of creation and answer of requests", exception.getMessage());
    verify(requestPeriodRepository).activePeriod();
  }
  ...
  void getPeriodById_WithNonExistingId_ShouldThrowResourceNotFoundException() {
    ...
    ResourceNotFoundException exception =
        assertThrows(
            ResourceNotFoundException.class, () -> requestPeriodService.getPeriodById(periodId));

    assertEquals(
        "There is no Request period identified with the id: " + periodId, exception.getMessage());
    verify(requestPeriodRepository).getById(periodId);
  }

  @Test
  void getAllPeriods_ShouldReturnAllPeriods() {
    List<RequestPeriodDTO> expectedPeriods =
        Arrays.asList(
            new RequestPeriodDTO(
                "1",
                ...
                false),
            new RequestPeriodDTO(
                "2", LocalDate.now().minusDays(5), LocalDate.now().plusDays(5), "2024", "2", true));
  ...
  void createActivePeriod_WhenNoActivePeriodExists_ShouldCreateNewPeriod() {
    CreateRequestPeriodDTO createDTO =
        new CreateRequestPeriodDTO(
            LocalDate.now().plusDays(1), LocalDate.now().plusDays(10), "2024", "1");

    when(requestPeriodRepository.activePeriod()).thenReturn(null);
    when(requestPeriodRepository.save(any(RequestPeriodDTO.class)))
  ...
  void createActivePeriod_WhenActivePeriodExists_ShouldThrowBusinessException() {
    ...
    RequestPeriodDTO existingPeriod =
        new RequestPeriodDTO(
    ...
    BusinessException exception =
        assertThrows(
            BusinessException.class, () -> requestPeriodService.createActivePeriod(createDTO));
  ...
  void updateActivePeriod_WithValidUpdates_ShouldUpdatePeriod() {
    ...
    UpdateRequestPeriodDTO updateDTO =
        new UpdateRequestPeriodDTO(
  ...
  void updateActivePeriod_WithNullUpdates_ShouldKeepOriginalValues() {
    ...
    RequestPeriodDTO existingPeriod =
        new RequestPeriodDTO("123", originalInitialDate, originalFinalDate, "2024", "1", true);
  ...
  void updateActivePeriod_WithSameValues_ShouldKeepOriginalValues() {
    ...
    UpdateRequestPeriodDTO updateDTO =
        new UpdateRequestPeriodDTO(originalInitialDate, originalFinalDate, true);
