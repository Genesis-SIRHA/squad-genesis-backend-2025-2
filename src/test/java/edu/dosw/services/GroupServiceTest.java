...
class GroupServiceTest {
    ...
    void addStudent_WhenHistorialAddFails_ShouldUpdateHistorial() {
        ...
        when(historialService.addHistorial(historialDTO)).thenThrow(new BusinessException("Historial error"));
        when(historialService.updateHistorial("STUD123", "GROUP1", HistorialStatus.ON_GOING)).thenReturn(mockHistorial);
    ...
    void addStudent_WhenBothHistorialOperationsFail_ShouldThrowBusinessException() {
        ...
        doNothing().when(groupValidator).validateAddStudentToGroup(group, "STUD123");
        when(groupRepository.save(group)).thenReturn(group);
        when(historialService.addHistorial(historialDTO)).thenThrow(new BusinessException("Add failed"));
        ...
        BusinessException exception = assertThrows(
                BusinessException.class,
                () -> groupService.addStudent("GROUP1", "STUD123")
        );
    ...
    void deleteStudent_WithValidData_ShouldReturnUpdatedGroup() {
        ...
        Historial mockHistorial = mock(Historial.class);
        when(historialService.updateHistorial(studentId, groupCode, HistorialStatus.CANCELLED)).thenReturn(mockHistorial);
    ...
    void deleteStudent_WithPeriodMismatch_ShouldThrowBusinessException() {
        ...
        BusinessException exception = assertThrows(
                BusinessException.class,
                () -> groupService.deleteStudent(groupCode, studentId)
        );
    ...
    void getCourseByGroupCode_WithEmptyGroupCode_ShouldThrowBusinessException() {
        BusinessException exception = assertThrows(
                BusinessException.class,
                () -> groupService.getCourseByGroupCode("")
        );
    ...
    void getCourseByGroupCode_WithNullGroupCode_ShouldThrowBusinessException() {
        BusinessException exception = assertThrows(
                BusinessException.class,
                () -> groupService.getCourseByGroupCode(null)
        );
    ...
    void getCourseByGroupCode_WithNonExistingGroupCode_ShouldThrowBusinessException() {
        ...
        when(facultyService.getAllFaculties()).thenReturn(Arrays.asList(faculty));
        when(groupRepository.findAllByCourseId("CS101")).thenReturn(Arrays.asList(createGroup("OTHER_GROUP")));

        BusinessException exception = assertThrows(
                BusinessException.class,
                () -> groupService.getCourseByGroupCode(groupCode)
        );
