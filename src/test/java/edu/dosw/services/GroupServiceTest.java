...
class GroupServiceTest {
  ...
  void deleteStudent_WhenHistorialUpdateFails_ShouldThrowBusinessException() {
    ...
    when(periodService.getPeriod()).thenReturn("1");
    when(groupRepository.save(group)).thenReturn(group);
    doThrow(new RuntimeException("Historial error"))
        .when(historialService)
        .updateHistorial("STUD123", "GROUP1", HistorialStatus.CANCELLED);

    BusinessException exception =
        assertThrows(
            BusinessException.class, () -> groupService.deleteStudent("GROUP1", "STUD123"));
