...
class RequestServiceTest {
    ...
    @BeforeEach
    void setUp() {
        mockRequest1 = new Request.RequestBuilder()
                .studentId("STU001")
        ...
        mockRequest2 = new Request.RequestBuilder()
                .studentId("STU002")
    ...
    void fetchRequests_ShouldThrowBusinessException_WhenUnsupportedRole() {
        assertThrows(BusinessException.class,
                () -> requestService.fetchRequests(Role.ADMINISTRATOR, "USER001"));
    }
    ...
    void updateRequest_ShouldUpdateAndReturnRequest_WhenValid() {
        when(requestRepository.findByRequestId("REQ001"))
                .thenReturn(Optional.of(mockRequest1));
        doNothing().when(validatorService)
                .validateUpdateRequest(anyString(), any(Request.class), any(UpdateRequestDto.class));
        when(answerStrategyFactory.getStrategy(RequestType.SWAP))
                .thenReturn(answerStrategy);
    ...
    void updateRequest_ShouldNotCallAnswerStrategy_WhenStatusNotAccepted() {
        UpdateRequestDto rejectedDto = new UpdateRequestDto(
                "REQ001",
    ...
    void deleteRequestStatus_ShouldThrowResourceNotFoundException_WhenNotFound() {
        when(requestRepository.findByRequestId("REQ999"))
                .thenReturn(Optional.empty());

        assertThrows(ResourceNotFoundException.class,
                () -> requestService.deleteRequestStatus("REQ999"));
    ...
    void deleteRequestStatus_ShouldThrowBusinessException_WhenDeleteFails() {
        ...
        doThrow(new RuntimeException("Delete failed"))
                .when(requestRepository).delete(mockRequest1);

        assertThrows(BusinessException.class,
                () -> requestService.deleteRequestStatus("REQ001"));
    }
    ...
    void getRequest_ShouldThrowResourceNotFoundException_WhenNotFound() {
        ...
        assertThrows(ResourceNotFoundException.class,
                () -> requestService.getRequest("REQ999"));
    }
    ...
    void updateRequest_ShouldUpdateManagedBy_WhenProvided() {
        ...
        verify(requestRepository).save(argThat(request ->
                request.getGestedBy().equals("DEAN001")
        ));
    }
    ...
    void updateRequest_ShouldUpdateAnswer_WhenProvided() {
        ...
        verify(requestRepository).save(argThat(request ->
                request.getAnswer().equals("Request approved")
        ));
    }
    ...
    void updateRequest_ShouldHandleJoinRequestType() {
        Request joinRequest = new Request.RequestBuilder()
                .studentId("STU003")
        ...
        UpdateRequestDto joinUpdateDto = new UpdateRequestDto(
                "REQ003",
                RequestStatus.ACCEPTED,
        ...
        when(requestRepository.findByRequestId("REQ003"))
                .thenReturn(Optional.of(joinRequest));
        doNothing().when(validatorService)
                .validateUpdateRequest(anyString(), any(Request.class), any(UpdateRequestDto.class));
        when(answerStrategyFactory.getStrategy(RequestType.JOIN))
                .thenReturn(answerStrategy);
        doNothing().when(answerStrategy).answerRequest(any(Request.class));
    ...
    void updateRequest_ShouldHandleCancellationRequestType() {
        Request cancellationRequest = new Request.RequestBuilder()
                .studentId("STU004")
                .type(RequestType.CANCELLATION)
        ...
        UpdateRequestDto cancellationUpdateDto = new UpdateRequestDto(
                "REQ004",
                RequestStatus.ACCEPTED,
        ...
        when(requestRepository.findByRequestId("REQ004"))
                .thenReturn(Optional.of(cancellationRequest));
        doNothing().when(validatorService)
                .validateUpdateRequest(anyString(), any(Request.class), any(UpdateRequestDto.class));
        when(answerStrategyFactory.getStrategy(RequestType.CANCELLATION))
                .thenReturn(answerStrategy);
        doNothing().when(answerStrategy).answerRequest(any(Request.class));
    ...
    void createRequest_ShouldHandleJoinRequestType() {
        CreateRequestDto joinRequestDto = new CreateRequestDto(
                "STU005",
                RequestType.JOIN,
    ...
    void createRequest_ShouldHandleCancellationRequestType() {
        CreateRequestDto cancellationRequestDto = new CreateRequestDto(
                "STU006",
                RequestType.CANCELLATION,
                "Request to cancel enrollment",
                null,
                "GROUP004"
        );
