...
class PemsumServiceTest {
    ...
    void getPemsum_ShouldThrowException_WhenNoCoursesFound() {
        ...
        when(studentService.getStudentById(STUDENT_ID)).thenReturn(student);
        when(facultyService.findCoursesByFacultyNameAndPlan(FACULTY_NAME, PLAN)).thenReturn(new ArrayList<>());
    ...
    void getCompletedCoursesPercentage_ShouldReturnPercentage() {
        ...
        when(studentService.getStudentById(STUDENT_ID)).thenReturn(student);
        when(facultyService.findCoursesByFacultyNameAndPlan(FACULTY_NAME, PLAN)).thenReturn(courses);
        when(historialService.getHistorialByStudentIdAndStatus(STUDENT_ID, HistorialStatus.FINISHED)).thenReturn(finishedHistorials);
    ...
    void getCompletedCoursesPercentage_ShouldThrowException_WhenNoCoursesFound() {
        ...
        assertThrows(ResourceNotFoundException.class, () -> pemsumService.getCompletedCoursesPercentage(STUDENT_ID));
    }
    ...
    void getCompletedCoursesPercentage_ShouldReturnZero_WhenNoApprovedCourses() {
        ...
        when(studentService.getStudentById(STUDENT_ID)).thenReturn(student);
        when(facultyService.findCoursesByFacultyNameAndPlan(FACULTY_NAME, PLAN)).thenReturn(courses);
        when(historialService.getHistorialByStudentIdAndStatus(STUDENT_ID, HistorialStatus.FINISHED)).thenReturn(new ArrayList<>());
    ...
    void getCompletedCoursesPercentage_ShouldHandleAllStatusTypes() {
        ...
        when(studentService.getStudentById(STUDENT_ID)).thenReturn(student);
        when(facultyService.findCoursesByFacultyNameAndPlan(FACULTY_NAME, PLAN)).thenReturn(courses);
        when(historialService.getHistorialByStudentIdAndStatus(STUDENT_ID, HistorialStatus.FINISHED)).thenReturn(mixedHistorials);
        when(groupService.getGroupByGroupCode("GRP001")).thenReturn(createGroup("MATH101"));
    ...
    void getStudentCoursesStatus_ShouldThrowException_WhenNoCoursesFound() {
        ...
        assertThrows(ResourceNotFoundException.class, () -> pemsumService.getStudentCoursesStatus(STUDENT_ID));
    }
    ...
    void getApprovedCredits_ShouldCalculateOnlyFinishedStatus() {
        ...
        PemsumService service = new PemsumService(facultyService, studentService, historialService, groupService);
