...
class DeanServiceTest {
    ...
    @BeforeEach
    void setUp() {
        validDeanDto = new DeanDto(
                "Juan Carlos Perez Gomez",
                "12345678",
                FACULTY_NAME
        );

        existingDean = new Dean.DeanBuilder()
                .userId(DEAN_ID)
    ...
    void generateDeanEmail_WithTwoPartName_ShouldThrowValidationException() throws Exception {
        ...
        Exception exception = assertThrows(Exception.class,
                () -> method.invoke(deanService, "Nombre Apellido"));
    ...
    void generateDeanEmail_WithSingleWordName_ShouldThrowValidationException() throws Exception {
        ...
        Exception exception = assertThrows(Exception.class,
                () -> method.invoke(deanService, "Unico"));
    ...
    void getDeanById_WhenDeanExists_ShouldReturnDean() {
        when(deanRepository.findByUserId(DEAN_ID))
                .thenReturn(Optional.of(existingDean));
    ...
    void getDeanById_WhenDeanNotFound_ShouldThrowException() {
        ...
        ResourceNotFoundException exception = assertThrows(
                ...
                () -> deanService.getDeanById(DEAN_ID)
        );
    ...
    void createDean_WithNullIdentityDocument_ShouldThrowValidationException() {
        ...
        jakarta.validation.ValidationException exception = assertThrows(
                ...
                () -> deanService.createDean(invalidDto)
        );
    ...
    void createDean_WhenAuthenticationServiceThrowsException_ShouldThrowBusinessException() {
        ...
        doThrow(new RuntimeException("Authentication service error"))
                .when(authenticationService).createAuthentication(any(Dean.class));

        BusinessException exception = assertThrows(
                BusinessException.class,
                () -> deanService.createDean(validDeanDto)
        );

        assertTrue(exception.getMessage().contains("An inesperated error has occurred when creating the dean"));
        verify(deanRepository, never()).save(any(Dean.class));
    }
    ...
    void updateDean_WhenDeanNotFound_ShouldThrowException() {
        ...
        ResourceNotFoundException exception = assertThrows(
                ResourceNotFoundException.class,
                () -> deanService.updateDean(DEAN_ID, validDeanDto)
        );
    ...
    void updateDean_WhenRepositoryThrowsException_ShouldThrowBusinessException() {
        ...
        assertTrue(exception.getMessage().contains("An inesperated error has occurred when updating the dean"));
        verify(deanRepository, times(1)).save(any(Dean.class));
    }
    ...
    void deleteDean_WhenAuthenticationDeletionFails_ShouldThrowBusinessException() {
        ...
        doThrow(new RuntimeException("Auth deletion failed"))
                .when(authenticationService).deleteAuthentication(existingDean);
        ...
        assertTrue(exception.getMessage().contains("An inesperated error has occurred when deleting the dean"));
    ...
    void getFacultyByDeanId_WhenDeanExists_ShouldReturnFacultyName() {
        when(deanRepository.findById(DEAN_ID))
                .thenReturn(Optional.of(existingDean));
    ...
    void getFacultyByDeanId_WhenDeanNotFound_ShouldThrowException() {
        ...
        ResourceNotFoundException exception = assertThrows(
                ResourceNotFoundException.class,
                () -> deanService.getFacultyByDeanId(DEAN_ID)
        );
    ...
    void createDean_WithSpecialCharactersInName_ShouldGenerateValidEmail() {
        DeanDto deanWithSpecialChars = new DeanDto(
                "José María Pérez",
                "99999999",
                FACULTY_NAME
        );
    ...
    void getFacultyByDeanId_WithDifferentDeanId_ShouldReturnCorrectFaculty() {
        ...
        Dean differentDean = new Dean.DeanBuilder()
        ...
        when(deanRepository.findById(differentDeanId))
                .thenReturn(Optional.of(differentDean));
