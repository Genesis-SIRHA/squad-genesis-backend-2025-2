...
class AdministratorServiceTest {
    ...
    @BeforeEach
    void setUp() {
        validAdministratorDto = new AdministratorDto(
                "12345678",
                "Juan Carlos Perez Gomez"
        );

        existingAdministrator = new Administrator.AdministratorBuilder()
                .userId(ADMIN_ID)
    ...
    @Test
    void generateAdministratorEmail_WithValidFourPartName_ShouldGenerateCorrectEmail() throws Exception {
        Method method = AdministratorService.class.getDeclaredMethod("generateAdministratorEmail", String.class);
        method.setAccessible(true);
    ...
    @Test
    void generateAdministratorEmail_WithUpperCaseName_ShouldGenerateLowerCaseEmail() throws Exception {
        Method method = AdministratorService.class.getDeclaredMethod("generateAdministratorEmail", String.class);
        method.setAccessible(true);
    ...
    @Test
    void generateAdministratorEmail_WithTwoPartName_ShouldThrowValidationException() throws Exception {
        Method method = AdministratorService.class.getDeclaredMethod("generateAdministratorEmail", String.class);
        method.setAccessible(true);

        Exception exception = assertThrows(Exception.class,
                () -> method.invoke(administratorService, "Nombre Apellido"));

        assertTrue(exception.getCause() instanceof jakarta.validation.ValidationException);
    }

    @Test
    void generateAdministratorEmail_WithSingleWordName_ShouldThrowValidationException() throws Exception {
        Method method = AdministratorService.class.getDeclaredMethod("generateAdministratorEmail", String.class);
        method.setAccessible(true);

        Exception exception = assertThrows(Exception.class,
                () -> method.invoke(administratorService, "Unico"));
    ...
    void updateAdministrator_WhenAdministratorExists_ShouldUpdateAllFieldsSuccessfully() {
        ...
        when(administratorRepository.save(any(Administrator.class)))
                .thenReturn(existingAdministrator);
    ...
    void updateAdministrator_WhenOnlyDocumentProvided_ShouldUpdateOnlyDocument() {
        ...
        Administrator result = administratorService.updateAdministrator(ADMIN_ID, documentOnlyUpdateDto);
    ...
    void updateAdministrator_WithUnexpectedError_ShouldThrowBusinessException() {
        ...
        assertEquals("An inesperated error has occurred when updating the administrator",
    ...
    void deleteAdministrator_WhenAuthenticationDeletionFails_ShouldThrowBusinessException() {
        ...
        doThrow(new RuntimeException("Auth deletion failed"))
                .when(authenticationService).deleteAuthentication(existingAdministrator);
        ...
        assertEquals("An inesperated error has occurred when deleting the administrator",
                exception.getMessage());
    ...
    void deleteAdministrator_WithUnexpectedError_ShouldThrowBusinessException() {
        when(administratorRepository.findByUserId(ADMIN_ID))
                .thenReturn(Optional.of(existingAdministrator));
        doNothing().when(authenticationService).deleteAuthentication(existingAdministrator);
        doThrow(new RuntimeException("Database error")).when(administratorRepository).delete(existingAdministrator);
