...
class CourseControllerTest {
    ...
    void getCourseById_WhenCourseExists_ShouldReturnCourse() {
        ...
        when(facultyService.findCourseByAbbreviation(courseAbbreviation, facultyName, plan)).thenReturn(expectedCourse);

        ResponseEntity<Course> response = courseController.getCourseById(courseAbbreviation, facultyName, plan);

        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertNotNull(response.getBody());
        assertEquals(courseAbbreviation, response.getBody().getAbbreviation());
        verify(facultyService, times(1)).findCourseByAbbreviation(courseAbbreviation, facultyName, plan);
    }
    ...
    void createCourse_WithValidData_ShouldReturnFaculty() {
        CourseRequest courseRequest = new CourseRequest("MATH101", "Mathematics", 4, "Engineering", "2024");
    ...
    void updateCourse_WithValidData_ShouldReturnUpdatedCourse() {
        ...
        when(facultyService.updateCourse(courseAbbreviation, facultyName, plan, updateDTO)).thenReturn(updatedCourse);

        ResponseEntity<Course> response = courseController.updateCourse(courseAbbreviation, facultyName, plan, updateDTO);
    ...
    void deleteCourseByCourseAbbreviation_WhenCourseExists_ShouldReturnNoContent() {
        ...
        ResponseEntity<Void> response = courseController.deleteCourseByCourseAbbreviation(courseAbbreviation, facultyName, plan);
    ...
    void deleteCourseByCourseAbbreviation_WhenCourseNotExists_ShouldThrowException() {
        ...
        doThrow(new RuntimeException("Course not found"))
                .when(facultyService).deleteCourse(courseAbbreviation, facultyName, plan);
