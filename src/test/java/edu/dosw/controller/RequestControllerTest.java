...
class RequestControllerTest {
    ...
    void createRequest_WithJoinType_ShouldReturnCreatedRequest() {
        ...
        Request createdRequest = new Request.RequestBuilder().studentId("STU001").type(RequestType.JOIN).description("Request to join math group").destinationGroupId("GROUP001").build();
    ...
    void createRequest_WithSwapType_ShouldReturnCreatedRequest() {
        CreateRequestDto createDto = new CreateRequestDto("STU001", RequestType.SWAP, "Request to swap groups", "GROUP001", "GROUP002");
        Request createdRequest = new Request.RequestBuilder().studentId("STU001").type(RequestType.SWAP).description("Request to swap groups").originGroupId("GROUP001").destinationGroupId("GROUP002").build();
    ...
    void createRequest_WithCancellationType_ShouldReturnCreatedRequest() {
        CreateRequestDto createDto = new CreateRequestDto("STU001", RequestType.CANCELLATION, "Request to cancel course", "GROUP001", null);
        Request createdRequest = new Request.RequestBuilder().studentId("STU001").type(RequestType.CANCELLATION).description("Request to cancel course").originGroupId("GROUP001").build();
    ...
    void createRequest_WithNullDescription_ShouldReturnCreatedRequest() {
        CreateRequestDto createDto = new CreateRequestDto("STU001", RequestType.JOIN, null, null, "GROUP001");
        Request createdRequest = new Request.RequestBuilder().studentId("STU001").type(RequestType.JOIN).destinationGroupId("GROUP001").build();
    ...
    void createRequest_WithEmptyDescription_ShouldReturnCreatedRequest() {
        CreateRequestDto createDto = new CreateRequestDto("STU001", RequestType.JOIN, "", null, "GROUP001");
        Request createdRequest = new Request.RequestBuilder().studentId("STU001").type(RequestType.JOIN).description("").destinationGroupId("GROUP001").build();
    ...
    void createRequest_WithLongDescription_ShouldReturnCreatedRequest() {
        ...
        CreateRequestDto createDto = new CreateRequestDto("STU001", RequestType.JOIN, longDescription, null, "GROUP001");
        Request createdRequest = new Request.RequestBuilder().studentId("STU001").type(RequestType.JOIN).description(longDescription).destinationGroupId("GROUP001").build();
    ...
    void createRequest_WithSpecialCharactersInDescription_ShouldReturnCreatedRequest() {
        CreateRequestDto createDto = new CreateRequestDto("STU001", RequestType.JOIN, "Request with spécial chàrâctérs!", null, "GROUP001");
        Request createdRequest = new Request.RequestBuilder().studentId("STU001").type(RequestType.JOIN).description("Request with spécial chàrâctérs!").destinationGroupId("GROUP001").build();
    ...
    void createRequest_WithDifferentStudentIds_ShouldReturnCreatedRequests() {
        CreateRequestDto createDto1 = new CreateRequestDto("STU001", RequestType.JOIN, "Request 1", null, "GROUP001");
        CreateRequestDto createDto2 = new CreateRequestDto("STU002", RequestType.JOIN, "Request 2", null, "GROUP002");

        Request createdRequest1 = new Request.RequestBuilder().studentId("STU001").type(RequestType.JOIN).description("Request 1").destinationGroupId("GROUP001").build();
        Request createdRequest2 = new Request.RequestBuilder().studentId("STU002").type(RequestType.JOIN).description("Request 2").destinationGroupId("GROUP002").build();
    ...
    void createRequest_WhenServiceThrowsException_ShouldPropagateException() {
        CreateRequestDto createDto = new CreateRequestDto("STU001", RequestType.JOIN, "Request", null, "GROUP001");
    ...
    void createRequest_VerifyAutoGeneratedFields() {
        CreateRequestDto createDto = new CreateRequestDto("STU001", RequestType.JOIN, "Request", null, "GROUP001");
        Request createdRequest = new Request.RequestBuilder().studentId("STU001").type(RequestType.JOIN).description("Request").destinationGroupId("GROUP001").build();
    ...
    void fetchRequests_ByStudentRole_ShouldReturnStudentRequests() {
        ...
        List<Request> expectedRequests = Arrays.asList(
                new Request.RequestBuilder().studentId("STU001").type(RequestType.JOIN).description("Join request").build(),
                new Request.RequestBuilder().studentId("STU001").type(RequestType.CANCELLATION).description("Cancel course").build()
        );
    ...
    void fetchRequests_ByProfessorRole_ShouldReturnProfessorRequests() {
        ...
        List<Request> expectedRequests = List.of(
                new Request.RequestBuilder().studentId("STU001").type(RequestType.JOIN).description("Join request for professor's group").build()
        );
    ...
    void fetchRequests_WhenServiceThrowsException_ShouldPropagateException() {
        String userId = "STU001";
        Role role = Role.STUDENT;
        when(requestService.fetchRequests(role, userId)).thenThrow(new RuntimeException("Service error"));
    ...
    void fetchRequests_WithDifferentUserIds_ShouldReturnDifferentResults() {
        ...
        List<Request> requests1 = List.of(new Request.RequestBuilder().studentId("STU001").type(RequestType.JOIN).build());
        List<Request> requests2 = List.of(new Request.RequestBuilder().studentId("STU002").type(RequestType.CANCELLATION).build());
    ...
    void fetchRequests_WithSpecialCharactersInUserId_ShouldHandleCorrectly() {
        ...
        Role role = Role.STUDENT;
        List<Request> expectedRequests = List.of(new Request.RequestBuilder().studentId("STU-001-ABC").type(RequestType.JOIN).build());
    ...
    void fetchRequests_WithLongUserId_ShouldHandleCorrectly() {
        ...
        Role role = Role.STUDENT;
        List<Request> expectedRequests = List.of(new Request.RequestBuilder().studentId("STU001234567890123456789").type(RequestType.JOIN).build());
    ...
    void fetchRequests_VerifyRoleSpecificLogic() {
        String userId = "PROF001";
        Role role = Role.PROFESSOR;
        List<Request> expectedRequests = List.of(new Request.RequestBuilder().studentId("STU001").type(RequestType.JOIN).build());
    ...
    void fetchAllRequests_WithLargeDataset_ShouldHandlePerformance() {
        List<Request> largeList = Arrays.asList(
                new Request.RequestBuilder().studentId("STU001").type(RequestType.JOIN).build(),
    ...
    void getRequest_ById_ShouldReturnRequest() {
        ...
        Request expectedRequest = new Request.RequestBuilder().studentId("STU001").type(RequestType.JOIN).description("Test request").build();
    ...
    void getRequest_WithDifferentStatuses_ShouldReturnRequest() {
        ...
        Request expectedRequest = new Request.RequestBuilder().studentId("STU002").type(RequestType.JOIN).build();
    ...
    void getRequest_WithSpecialRequestId_ShouldHandleCorrectly() {
        ...
        Request expectedRequest = new Request.RequestBuilder().studentId("STU001").type(RequestType.JOIN).build();
    ...
    void getStudentHistorialRequests_ShouldReturnStudentRequests() {
        ...
        List<Request> expectedRequests = Arrays.asList(
                ...
                new Request.RequestBuilder().studentId(studentId).type(RequestType.CANCELLATION).build()
        );

        when(requestService.fetchRequests(Role.STUDENT, studentId)).thenReturn(expectedRequests);

        ResponseEntity<List<Request>> response = requestController.getStudentHistorialRequests(studentId);
    ...
    void getStudentHistorialRequests_WhenServiceThrowsException_ShouldPropagateException() {
        String studentId = "STU001";
        when(requestService.fetchRequests(Role.STUDENT, studentId)).thenThrow(new RuntimeException("Service error"));

        assertThrows(RuntimeException.class, () -> requestController.getStudentHistorialRequests(studentId));
        verify(requestService, times(1)).fetchRequests(Role.STUDENT, studentId);
    }
    ...
    void getStudentHistorialRequests_WithDifferentStudents_ShouldReturnDifferentResults() {
        ...
        List<Request> requests1 = List.of(new Request.RequestBuilder().studentId(studentId1).type(RequestType.JOIN).build());
        List<Request> requests2 = List.of(new Request.RequestBuilder().studentId(studentId2).type(RequestType.CANCELLATION).build());
        ...
        ResponseEntity<List<Request>> response1 = requestController.getStudentHistorialRequests(studentId1);
        ResponseEntity<List<Request>> response2 = requestController.getStudentHistorialRequests(studentId2);
    ...
    void getStudentHistorialRequests_ShouldAlwaysUseStudentRole() {
        String studentId = "STU001";
        List<Request> expectedRequests = List.of(new Request.RequestBuilder().studentId(studentId).type(RequestType.JOIN).build());
    ...
    void getRequestByFacultyName_WhenServiceThrowsException_ShouldPropagateException() {
        String facultyName = "engineering";
        when(requestService.fetchRequestsByFacultyName(facultyName)).thenThrow(new RuntimeException("Service error"));

        assertThrows(RuntimeException.class, () -> requestController.getRequestByFacultyName(facultyName));
        verify(requestService, times(1)).fetchRequestsByFacultyName(facultyName);
    }
    ...
    void getRequestByFacultyName_WithSpecialCharacters_ShouldHandleCorrectly() {
        ...
        when(requestService.fetchRequestsByFacultyName("computer-science")).thenReturn(expectedRequests);
