...
class RequestControllerTest {
  ...
  void getRequestByFacultyName_WithSpecialCharacters_ShouldHandleCorrectly() {
    ...
    @Test
    void getMateriaReassignmentStats_ShouldReturnReportDTO() {
        ReportDTO expectedReport = new ReportDTO(50L, 10L, 30L, 10L, 20L, 25L, 25L);
        when(requestService.getMateriaReassignmentStats()).thenReturn(expectedReport);

        ResponseEntity<ReportDTO> response = requestController.getMateriaReassignmentStats();
        ...
        verify(requestService, times(1)).getMateriaReassignmentStats();
    }

    @Test
    void getGrupoReassignmentStats_ShouldReturnReportDTO() {
        ReportDTO expectedReport = new ReportDTO(40L, 5L, 30L, 5L, 10L, 15L, 5L);
        when(requestService.getGrupoReassignmentStats()).thenReturn(expectedReport);

        ResponseEntity<ReportDTO> response = requestController.getGrupoReassignmentStats();

        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertEquals(expectedReport, response.getBody());
        assertEquals(40L, response.getBody().total());
        assertEquals(30L, response.getBody().approved());
        verify(requestService, times(1)).getGrupoReassignmentStats();
    }

    @Test
    void getDecanaturaReassignmentStats_ShouldReturnReportDTO() {
        ReportDTO expectedReport = new ReportDTO(15L, 3L, 10L, 2L, 4L, 5L, 1L);
        when(requestService.getDecanaturaReassignmentStats()).thenReturn(expectedReport);

        ResponseEntity<ReportDTO> response = requestController.getDecanaturaReassignmentStats();

        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertEquals(expectedReport, response.getBody());
        assertEquals(15L, response.getBody().total());
        assertEquals(10L, response.getBody().approved());
        verify(requestService, times(1)).getDecanaturaReassignmentStats();
    }

    @Test
    void getGlobalReassignmentStats_ShouldReturnReportDTO() {
        ReportDTO expectedReport = new ReportDTO(100L, 20L, 70L, 10L, 30L, 40L, 30L);
        when(requestService.getGlobalReassignmentStats()).thenReturn(expectedReport);

        ResponseEntity<ReportDTO> response = requestController.getGlobalReassignmentStats();

        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertEquals(expectedReport, response.getBody());
        assertEquals(100L, response.getBody().total());
        assertEquals(70L, response.getBody().approved());
        verify(requestService, times(1)).getGlobalReassignmentStats();
    }

    @Test
    void getMateriaReassignmentStats_WhenServiceReturnsEmpty_ShouldReturnEmptyReport() {
        ReportDTO emptyReport = new ReportDTO(0L, 0L, 0L, 0L, 0L, 0L, 0L);
        when(requestService.getMateriaReassignmentStats()).thenReturn(emptyReport);
    ...
    @Test
    void getAllReportEndpoints_ShouldCallServiceExactlyOnce() {
        ReportDTO report = new ReportDTO(10L, 2L, 6L, 2L, 3L, 4L, 3L);
        when(requestService.getMateriaReassignmentStats()).thenReturn(report);
        when(requestService.getGrupoReassignmentStats()).thenReturn(report);
        when(requestService.getDecanaturaReassignmentStats()).thenReturn(report);
        when(requestService.getGlobalReassignmentStats()).thenReturn(report);

        requestController.getMateriaReassignmentStats();
        requestController.getGrupoReassignmentStats();
        requestController.getDecanaturaReassignmentStats();
        requestController.getGlobalReassignmentStats();
    ...
    @Test
    void getReportEndpoints_WhenServiceThrowsException_ShouldPropagateException() {
        when(requestService.getMateriaReassignmentStats()).thenThrow(new RuntimeException("Service error"));

        assertThrows(RuntimeException.class, () -> requestController.getMateriaReassignmentStats());
        verify(requestService, times(1)).getMateriaReassignmentStats();
    }
}
